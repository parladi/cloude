    ...
    <aside className={`bg-brand-100 border-r border-brand-200 transition-all duration-200 flex flex-col ${open ? 'w-64' : 'w-16'}`}>
      <div className="p-4 border-b border-brand-200">
        <div className="flex items-center gap-2">
          ...
          {open && <div><h1 className="text-sm font-bold text-brand-700 leading-tight">TIGER</h1><p className="text-xs text-text-secondary leading-tight">Denetim Paneli</p></div>}
        </div>
      </div>
      <nav className="flex-1 py-2">
        {menuItems.map(item => (
          <NavLink key={item.path} to={item.path} className={({ isActive }) => `flex items-center gap-3 px-4 py-2.5 mx-2 my-0.5 rounded-lg text-sm transition-all duration-200 ${isActive ? 'bg-brand-500 text-white shadow-sm' : 'text-text-primary hover:bg-brand-200/50'}`}>
      ...
      {open && <div className="p-4 border-t border-brand-200 text-xs text-text-secondary"><p>{connInfo?.server || '-'}</p><p>{connInfo?.database || '-'}</p></div>}
    </aside>
  );
}
