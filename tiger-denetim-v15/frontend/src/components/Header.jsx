export default function Header({ connected, connInfo, onRefresh }) {
  return (
    <header className="bg-brand-500 text-white h-16 flex items-center justify-between px-6 shadow-md">
      <div>
        ...
        <p className="text-xs text-brand-200">{connInfo?.server || '-'} - {connInfo?.database || '-'} / Firma 321</p>
      </div>
      <div className="flex items-center gap-3">
        <button onClick={onRefresh} className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 px-4 py-2 rounded-lg text-sm transition-all duration-200">
